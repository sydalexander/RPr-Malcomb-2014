...
author: "Joseph Holler, Kufre Udoh, Drew An-Pham, Sydney Alexander, Middlebury Open GIScience Classes"
date: "`r Sys.Date()`"
...
vuln_map
```
### Turn Vulnerability Raster into TA zones

**Planned deviation**
We make a new map visualizing mean vulnerability scores for TAs using the ***aggregate*** function, contrasting the visualization of Fig 5. 
Change the scale fill from continuous to "binned."

```{r compute zonal stats}
#calculate mean vulnerability from raster in TAs
mean_Vuln_TAS <- aggregate(vulnerability_r, ta_v, FUN = mean) %>%
  st_as_sf()

#visualize the TA vulnerabilities 
# ta_vuln_map <-
#ggplot() + 
 # geom_sf(
  #  data = mean_Vuln_TAS, 
...
#  ) +
 # scale_fill_continuous(
  #  name = "Vulnerability Mean/Scores", 
   # low="lemonchiffon1", 
  #  high="lightcoral"
#  ) +
 # scale_y_continuous(breaks=seq(-17,-10,1)) +
...
The purpose of this study was to reproduce Malcomb's Vulnerability Modeling Study in Malawi, and to test the effectiveness of our reproduction.
We used Spearmans' Rho Correlation to determine if our results exactly matched the original author's results. 
A failure to reject these hypotheses would indicate that our results did not exactly match those of the original authors.
A positive correlation approaching 1 would indicate a partial reproduction.

Some variation from the original study was inevitable, given that some of the data sources from the original study were unavailable, including DHS data. 
We calculated **adaptive capacity** and **vulnerability**,
comparing our scores to the values calculated in the original study. 

## Adaptive capacity

The Spearman's Rho correlation coefficient was 0.7891, indicating that there is a strong positive correlation between the adaptive capacity scores of the original study and of our reproduction. Essentially, our reproduction models had values which were fairly similar to the values of the original model. This means we fail to reject the hypothesis that we would be unable to perfectly replicate Malcomb's original study with regards to adaptive capacity.

We also mapped the differences between the original study's adaptive capacity scores and the adaptive capacity scores calculated in our reproduction, subtracting scores of the original from the reproduction. Within TAs, the differences were often negative, indicating that there were many instances in which our reproduction predicted values smaller than the original.


##Vulnerability

The Spearman's Rho correlation coefficient for vulnerability scores was 0.197, indicating that there was a weak positive correlation between the vulnerability scores of the original study and of the reproduction. This value indicates that we were unable to perfectly replicate the author's model, nor were we very close in finding the same values. 

The graph comparing vulnerability scores between the original study and the reproduction illustrates that there is little correlation; the data points transposed from the graph to not adhere to any particular pattern, and are widely scattered across the graph.

After mapping the differences in vulnerability scores from the original study's raster to the reproduction of Figure 5, it appears at a quick glance that the original generally expected higher values of vulnerability, especially in the southern regions of Malawi. 

##New Visualization for Vulnerability
One modification we made to the original study was changing one of Malcomb's visualizations. Originally, he had vulnerability mapped as a raster, showing a continuous range of scores (Fig 5). However, this is not the best way to visualize the data: stakeholders or policymakers may be able to implement policies for specific recognized areas, such as TAs, but it is a lot harder to do so with a raster of all of Malawi, with no smaller boundaries indicated. As such, we made a new map visualizing the mean vulnerability scores of each TA. Esentially, all of the vulnerability values were averaged out per TA, and the mean vulnerability value was reported. Additionally, given that this data is not continuous, we binned the scale bar. With this new visualization, it becomes clear that there are certain TAs with higher vulnerability, rather than ambiguous areas. 

##Reproducibility

The reproduction study used the indicators and weights as they were outlined in the original study.
One of the bigger issues with Malcomb's choice to black box the interview process and the way by which he created weights for his different variables.
It is much more difficult to make adjustments to weights, given the fact that we do not have access to Malcomb's methods for creating weights. 

There are other issues with reproducibility as well. We were unable to get the exact same results as Malcbomb's original study, although for some variables (eg. Adaptive Capacity) we were able to calculate closer values than others.

More generally, an issue we faced while replicating Malcomb's study were vague methods outlined in the methodology section of his paper.
There were not clear descriptions of how Malcomb's figures were produced. 

# Conclusions

We were unable to reject the working hypotheses that our reproduction would deviate from the original study given our correlation coefficients. For adaptive capacity and vulnerability, both of our Spearmen correlation coefficients were below 1, meaning that we were unable to perfectly replicate the original study.
However, the Spearman's correlation coefficent was much higher for adaptive capacity than for vulnerability, meaning that our reproduction of adaptive capacity was more robust than for vulnerability. 
The values for both adaptive capacity and vulnerability values for our reproduction tended to be smaller than the original values calculated in the original study.

Regarding its significance within the greater context of reproducibility for GIS science, there are a couple different implications for Malcomb's study.
Malcomb's study is supposed to be one that is fairly easily reproducible.
However, we were able to reproduce the figures from the original study in a new computational environment, we were unable to perfecly reproduce its results. 
We were able to implement unplanned deviations that we felt would improve the study, including different visualization choices.
However, there were several obstacles preventing a perfect reproduction.
For one, access to data was one hindrance that made a perfect reproduction fairly difficult.
Unclear methodology outlined by Malcomb also made a reproduction of his study more difficult.
For one, there was little elaboration on how he produced his figures.
There were several subjective decisions made throughout Malcomb's study which were not elaborated upon, such as the decisions made to create weights based on the results from the interviews.
If reproducible research is becoming the norm for geographic research, then our reproduction of Malcbomb's study illustrates the significance of methodology that is thorough and easy to follow.
In this case, it is not quite right to call a study reproducible if it does not have all the proper elements of a reproducible study. 
In this regard, access to original data sources is also important; it is very difficult to reproduce research if the data sources are not easily accessible.
These two elements in Malcomb's study (or lack thereof) illustrate the benefits of using a research compendium: putting data, and clear methodolgy all in one place could improve reproducibility by allowing future researchers to find all the necessary components in one place. 
